...
export default async function handler(req, res) {
  try {
    ...
    try {
      ...
      // If assistant didn't return strict JSON, attempt to extract JSON substring
      const maybeJson = assistant && assistant.match(/\{[\s\S]*\}/);
      if (maybeJson) {
        try {
          const parsed2 = JSON.parse(maybeJson[0]);
