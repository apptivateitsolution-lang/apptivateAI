    ...
    <div style={{fontFamily:'Inter, ui-sans-serif, system-ui', padding:20, maxWidth:1000, margin:'0 auto'}}>
      <header style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20}}>
        ...
        <div style={{display:'flex', gap:8}}>
          <button style={{padding:'8px 12px', borderRadius:8, border:'1px solid #e2e8f0', background:'#fff'}}>Docs</button>
          <button style={{padding:'8px 12px', borderRadius:8, background:'#0ea5e9', color:'#fff', border:'none'}}>Upgrade</button>
        </div>
      </header>
        ...
        <section style={{background:'#f8fafc', padding:16, borderRadius:12}}>
          ...
          {tab === 'post' && (
            <div>
              ...
              <div style={{marginTop:12}}><button onClick={()=>{const topic=document.getElementById('topic').value; callAI('post',{topic});}} style={{padding:'10px 16px', borderRadius:8, background:'#0ea5e9', color:'#fff', border:'none'}}>{loading ? '...' : 'Create Post'}</button></div>
            </div>
          )}
          ...
          {tab === 'audit' && (
            <div>
              ...
              <input value={site} onChange={(e)=>setSite(e.target.value)} placeholder="https://example.com" style={{width:'100%', padding:10, borderRadius:8, marginTop:8, border:'1px solid #e2e8f0'}} />
              <div style={{marginTop:12}}><button onClick={()=>callAI('audit',{website:site})} style={{padding:'10px 16px', borderRadius:8, background:'#0ea5e9', color:'#fff', border:'none'}}>{loading ? '...' : 'Run Audit'}</button></div>
            </div>
          )}
          ...
          {tab === 'message' && (
            <div>
              ...
              <textarea value={messageCtx} onChange={(e)=>setMessageCtx(e.target.value)} placeholder="Write a short promotional DM for our new product..." style={{width:'100%', padding:10, borderRadius:8, marginTop:8, border:'1px solid #e2e8f0', minHeight:120}} />
              <div style={{marginTop:12}}><button onClick={()=>callAI('message',{context:messageCtx})} style={{padding:'10px 16px', borderRadius:8, background:'#0ea5e9', color:'#fff', border:'none'}}>{loading ? '...' : 'Write Message'}</button></div>
            </div>
          )}
        </section>
